...
class Node:
        ...
        while True:
            ...
            try:
                ...
                try:    
                    ...
                    newSuccFound = False
                    for key, value in self.fingerTable.items():
                        if value[1] != self.succ and value[1] != succ_prev:
                            succ_now = value[1]
                            newSuccFound = True
                            break
                    if newSuccFound:
    ...
    def uploadFile(self, filename, recvIPport, replicate):
        ...
        try:
            ...
            cSocket.connect(recvIPport)
            data_pred = [6]             
            cSocket.sendall(pickle.dumps(data_pred))
            pred_IPport =  pickle.loads(cSocket.recv(buffer))
            print(pred_IPport)
            cSocket.connect(pred_IPport)
            cSocket.sendall(pickle.dumps(sDataList))
            cSocket.close()
    ...
    def downloadFile(self, filename):
        ...
        sSocket.connect(recvIPport)
        data_pred = [6]
        sSocket.sendall(pickle.dumps(data_pred))
        pred_IPport =  pickle.loads(sSocket.recv(buffer))
        print("akshatttt", pred_IPport)
        DSocket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        DSocket.connect(pred_IPport)
        ...
        if fileData == b"NotFound":
            print("File not found in Succ checking predicessor:", filename)
            DSocket.sendall(pickle.dumps(sDataList))
            fileData1 = DSocket.recv(buffer)
            if fileData1 == b"NotFound":
                print("File not found in predicessor too:", filename)
            else:
                print("Receiving file from predicessor:", filename)
                self.receiveFile(DSocket, filename)
        else:
            print("Receiving file:", filename)
